...
const transitionEndEventName = transitionEndEventNames[Modernizr.prefixed('transition')];
...
const advanceShow = function() {
  ...
  return $(visibleSet["wrap"]).on(transitionEndEventName, function() {
    ...
    if (nextItem) { return setMedia(hiddenSet, nextItem); }
  });
};
...
const initMap = function(set, item) {
  ...
  const myPano = new google.maps.StreetViewPanorama(set["map"], panoramaOptions);
...
const getYoutubeIframe = id => `<iframe width='420' height='315' src='//www.youtube.com/embed/${id}?rel=0&loop=1&autoplay=1&controls=0&playsinline=1&modestbranding=1&playlist=${id}' frameborder='0'></iframe>`;
...
var toggleChapterList = function() {
  ...
  if (chaptersList.classList.contains('is-hidden')) {
    ...
    return $(window).on('keyup.chapters', function(e) {
      if (e.keyCode === 27) { return toggleChapterList(); }
    }); // esc
  }
};
...
$(function() {
  ...
  return $('.seek-bar').on('click', function(ev) {
    ...
    if (x) { return seekVideoByRatio(x / this.offsetWidth); }
  });
});